...
class Scoreboard:
    ...
    def __init__(self, batch, group):
        ...
        self.font_size = 16
        self.distance_to_exit_label = pyglet.text.Label(
            "Direct Distance To Exit : 0",
            x=0,
            y=0,
            font_name="Arial",
        ...
        for index, player in enumerate(config_data.player_data):
            ...
            traveled_distance_label = pyglet.text.Label(
                "Distance Traveled:",
                x=0,
                y=0,
                font_name="Arial",
            ...
            excess_distance_label = pyglet.text.Label(
                "Excess Distance Traveled:",
                x=0,
                y=0,
                font_name="Arial",
                font_size=self.font_size,
    ...
    def update_elements_locations(self):
        ...
        for index, (display_element, player) in enumerate(self.player_name_display):
            ...
            display_element.y = (
                ...
                - self.stat_height * 2
                - self.stat_height * (index * self.number_of_stats)
            )
        ...
        for index, (display_element, player) in enumerate(
        ...
        for index, (display_element, player) in enumerate(
            self.player_nodes_visited_display
        ):
            display_element.x = config_data.window_width - self.stat_width
            display_element.y = (
                config_data.window_height
    ...
    def update_paths(self):
        for index in range(len(config_data.player_data)):
            self.player_path_display[index][0].text = self.wrap_text(
                str(global_game_data.graph_paths[index])
            )
    ...
    def update_distance_to_exit(self):
        ...
        self.distance_to_exit = math.sqrt(
            pow(start_x - end_x, 2) + pow(start_y - end_y, 2)
        )
        self.distance_to_exit_label.text = (
            "Direct Distance To Exit : " + "{0:.0f}".format(self.distance_to_exit)
        )
        ...
        for display_element, player_configuration_info in self.player_traveled_display:
            for player_object in global_game_data.player_objects:
                if player_object.player_config_data == player_configuration_info:
                    display_element.text = "Distance Traveled: " + str(
                        int(player_object.distance_traveled)
                    )

        for (
            display_element,
            player_configuration_info,
        ) in self.player_excess_distance_display:
            for player_object in global_game_data.player_objects:
                if player_object.player_config_data == player_configuration_info:
                    display_element.text = "Excess Distance Traveled: " + str(
                        max(
                            0,
                            int(
                                player_object.distance_traveled - self.distance_to_exit
                            ),
                        )
                    )
    ...
    def update_nodes_visited(self):
        for (
            display_element,
            player_configuration_info,
        ) in self.player_nodes_visited_display:
            for player_object in global_game_data.player_objects:
                if player_object.player_config_data == player_configuration_info:
                    display_element.text = "Nodes Visited: " + str(
                        len(
                            global_game_data.graph_paths[
                                global_game_data.player_objects.index(player_object)
                            ]
                        )
                    )
